...
# ==============================================================================
# PROJECT AEGIS - FOREX EDITION (v4)
#
...
#   1) AegisTrader_Forex_Day   -> Target_Day_Win
#   2) AegisTrader_Forex_Swing -> Target_Swing_Win
#   3) AegisTrader_Forex_Long  -> Target_Long_Win
# ==============================================================================

ALGORITHM_FAMILY_NAME = "AegisTrader_Forex"
DATASET_NAME = "FOREX"
...
DATASET_PATH = (
    ...
    r"\DATASET_FOREX_ML_READY_v3.csv"
)
...
HORIZON_CONFIGS: Dict[str, Dict[str, Any]] = {
    "Day": {
        ...
        "model_name": "AegisTrader_Forex_Day",
        "description": "Short-term forex trading model for daily signals.",
        "look_forward": 3,
        "atr_mult": 1.5,
        "reward_to_risk": 2.0,
        "purge_gap_days": 3,
    ...
    "Swing": {
        ...
        "model_name": "AegisTrader_Forex_Swing",
        "description": "Medium-term forex trading model for within-week trading signals.",
        "look_forward": 14,
        "atr_mult": 2.5,
        "reward_to_risk": 3.0,
        "purge_gap_days": 14,
    ...
    "Long": {
        ...
        "model_name": "AegisTrader_Forex_Long",
        "description": "Long-term forex trading model for long-horizon positions.",
        "look_forward": 60,
        "atr_mult": 4.0,
        "reward_to_risk": 4.0,
        "purge_gap_days": 60,
...
try:
    ...
    raise ImportError(
        "Could not import 'ml_preprocessing_pipeline_v4.py'. Place it in the same folder as model_training_forex_v4.py."
    ) from exc
...
def make_report_text(results: List[Dict[str, Any]]) -> str:
    lines: List[str] = []
    lines.append("=" * 100)
    lines.append("PROJECT AEGIS - FOREX EDITION (v4)")
...
def main() -> None:
    ...
    print_section_separator()
    print(f"[*] Loading FOREX dataset from explicit DATASET_PATH")
    print(f"    Path: {DATASET_PATH}")
    if not os.path.exists(DATASET_PATH):
        raise FileNotFoundError(f"FOREX dataset was not found at: {DATASET_PATH}")
    ...
    report_path = os.path.join(OUTPUT_BASE_DIR, f"Forex_Model_Training_Evaluation_Results_v4_{timestamp}.txt")
    json_path = os.path.join(OUTPUT_BASE_DIR, f"Forex_Model_Training_Summary_v4_{timestamp}.json")
